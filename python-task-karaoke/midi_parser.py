...
class MidiParser:
    ...
    EXAMPLES:
         ...
         simple_karaoke_text.py shows how to use pygame to build a graphic frontend for
    ...
    def __init__(self):
        ...
        self.bpm = [[120, 0.0]]  # bpm using actual time signature
        self.microsecondsperquarternote = [[60000000.0 / 120, 0.0]]
        self.num = [[4, 0.0]]
        self.den = [[4, 0.0]]
        # For karaoke .kar files
        self.karfile = True
    ...
    def load_file(self, fileobject):
        ...
        division = struct.unpack(">h", self.fileobject.read(2))[
            0
        ...
        for itrack in range(self.ntracks):
            ...
            while iread < tracklen and metatype != 0x2F:
                ...
                secondspertick = 1.0 / tickspermicrosecond * 1e-6
                ...
                if i1 != i0:  # Tempo has changed. Recompute using MIDI steps
                    tickspermicrosecond = (
                        division / self.microsecondsperquarternote[i0][0]
                    )
                    secondspertick0 = 1.0 / tickspermicrosecond * 1e-6
                    tickspermicrosecond = (
                        division / self.microsecondsperquarternote[i1][0]
                    )
                    secondspertick1 = 1.0 / tickspermicrosecond * 1e-6
                    dtimesec = 0.0
                    for itick in range(dtime):
                        if (
                            mastertime + dtimesec
                            < self.microsecondsperquarternote[i1][1]
                        ):
                ...
                if status == 0xFF:  # It's a non-MIDI event, META event
                    ...
                    if metatype == 0x51:  # Set tempo
                        ...
                        self.microsecondsperquarternote.append(
                            [tt[0] * 65536.0 + tt[1] * 256.0 + tt[2], mastertime]
                        )
                        self.bpm.append(
                            [
                                60000000.0
                                / self.microsecondsperquarternote[-1][0]
                                * (self.den[-1][0] / self.num[-1][0]),
                                mastertime,
                            ]
                        )
                    ...
                    if metatype == 0x58:  # Time signature
                        ...
                        self.microsecondsperquarternote.append(
                            [self.microsecondsperquarternote[-1][0], mastertime]
                        )
                        self.bpm.append([self.bpm[-1][0], mastertime])
                    if metatype == 0x1:
                        data_dec = data.decode("cp1251")
                        if data_dec == "@KMIDI KARAOKE FILE":
                            self.karfile = True
                            self.kartrack = itrack + 1
                        else:
                            self.kartrack = itrack
                        if self.karfile and itrack == self.kartrack:
                            if len(data_dec) > 0 and data_dec[0] != "@":
                                if "\\" in data_dec:
                                    self.karsyl.append("\\")
                                    self.kartimes.append(mastertime)
                                    data = re.sub("\\\\", "", data_dec)
                                if "/" in data_dec:
                                    self.karsyl.append("/")
                                    self.kartimes.append(mastertime)
                                    data = re.sub("/", "", data_dec)
                                self.karsyl.append(data_dec)
                                self.kartimes.append(mastertime)
                    ...
                    if status1 == 0b1001 and data2 > 0:  # Note on event
                        ...
                        inote = (
                            len(self.notes) - 1
                        )  # If it was previously on, count it off
                        while inote >= 0:
                            if (
                                self.notes[inote][0] == data1
                                and self.notes[inote][2] == currentpatch
                            ):
                                self.notes[inote][5] == mastertime  # Time note off
                                break
                            inote = inote - 1
                    elif status1 == 0b1000 or (
                        status1 == 0b1001 and data2 == 0
                    ):  # Note off event
        ...
        if dt >= max(self.kartimes):
            for iline in range(-2, 1):
                if self.karlinea[iline] != "":
                    self.karlinea[iline] = "".join(
                        self.karsyl[self.karievent0[iline] :]
                    )
                    self.karlinea[iline] = re.sub("\\\\", "", self.karlinea[iline])
                    self.karlinea[iline] = re.sub("/", "", self.karlinea[iline])
                    self.karlineb[iline] = ""
